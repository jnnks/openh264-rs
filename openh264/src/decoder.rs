...
impl<'a> DecodedYUV<'a> {
    ...
    pub(crate) fn write_rgb8_f32x8(y_plane: &[u8], u_plane: &[u8], v_plane: &[u8], dim: (usize, usize), strides: (usize, usize, usize), target: &mut [u8]) {
        ...
        for y in 0..(height / 2)  {
            ...
            let base_y = (y * 2 + 1) * strides.0;
            let y_row = &y_plane[base_y..base_y+strides.0];

            // calculate second RGB row
            let base_tgt = (y * 2 + 1) * rgb_bytes_per_row;
            let row_target = &mut target[base_tgt..(base_tgt + rgb_bytes_per_row)];
            Self::write_rgb8_f32x8_row(&y_row, &u_row, &v_row, width, row_target);
        }
    }
    ...
    fn write_rgb8_f32x8_row(y_row: &[u8], u_row: &[u8], v_row: &[u8], width: usize, target: &mut [u8]) {
        ...
        for _ in (0..width).step_by(STEP) {
            ...
            let y_pack: wide::f32x8 = f32x8_y_from!(y_row, base_y);
            let u_pack: wide::f32x8 = f32x8_uv_from!(u_row, base_uv) - 128.0;
            let v_pack: wide::f32x8 = f32x8_uv_from!(v_row, base_uv) - 128.0;
